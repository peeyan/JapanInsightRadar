...
export function Header({ isRumorMode, setIsRumorMode }: HeaderProps) {
  return (
    <header className="flex justify-between items-center mb-12 border-b border-gray-500/30 pb-6">
      ...
      <div className="flex items-center gap-3">
        <span className={`text-sm font-bold ${!isRumorMode ? 'text-accent' : 'text-textSecondary'}`}>FACT</span>
        <button
        ...
        <span className={`text-sm font-bold ${isRumorMode ? 'text-accent' : 'text-textSecondary'}`}>RUMOR</span>
      </div>
    </header>
  );
}